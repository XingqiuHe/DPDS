...
parser.add_argument('--Alg', default='d_ddpg', type=str)
...
param.lam_init = np.ones(param.N) * 10000
param.theta = lambda t: 0
#param.theta = lambda t: min(10, 100/np.log(t+10))
...
class DPDS:
    ...
    def cost(self, batch_state, batch_action):
        ...
        #cost = tf.reduce_sum(-d*aoi_per_bit + self.lam * tf.math.maximum(0.0, E - self.E_max), axis=1)
    ...
    def train(self, s, a, r, s_next):
        ...
        with tf.GradientTape() as tape:
            ...
            target_a_next[:,:,2] = tf.nn.softmax(target_a_next[:,:,2])
            target_y = self.cost(s,a) + args.discount * self.target_critic([s_next, target_a_next], training=True)
        ...
        with tf.GradientTape() as tape:
            ...
            actions[:,:,2] = tf.nn.softmax(actions[:,:,2])
            critic_value = self.critic([s, actions], training=True)
            actor_loss = tf.math.reduce_mean(critic_value)

        actor_grad = tape.gradient(actor_loss, self.actor.trainable_variables)
        actor_grad = [tf.clip_by_norm(grad, 1.0) for grad in actor_grad]
